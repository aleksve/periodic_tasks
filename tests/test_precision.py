...
from periodic_tasks import make_periodic, run_loop, reset
...
def test():
    stop_event = Event()
    make_periodic(partial(set_event_after, t=10, event=stop_event), interval=1)
    make_periodic(check_time_slip, interval=_PERIOD)
    run_loop(stop_event)
